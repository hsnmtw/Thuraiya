...
namespace Thuraiya
{
    public class Nationalities : RTLForm
    {
        
		//private string[]columns;
		private DataGridView dg;
		public Nationalities() : base() {
            Text = ar("Nationalities");
		...
		{
			if(new NationalityDialog().ShowDialog()==DialogResult.OK){
				ReloadData();
			}
		}
		...
		public void EditRecordAction(object s, EventArgs ea)
		{
			...
			if(new NationalityDialog(dg.SelectedRows[0].Cells[0].Value.ToString()).ShowDialog()==DialogResult.OK){
				ReloadData();
			}
