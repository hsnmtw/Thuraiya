...
namespace Thuraiya
{
    public class PaySlips : RTLForm
    {
		...
		private void ReloadData() {
			var sql = @"select * from thrdb.payslip;";
		...
		private void Initialize() {
			...
			Button bAdd = new Button() { Name = "bAdd", Text = ar("Add"), Enabled=false,  Size=new Size(75,25), Location = new Point(10, 5) };
		...
		{
			if(new PaySlipDialog().ShowDialog()==DialogResult.OK){
				ReloadData();
			}
		}
		...
		public void EditRecordAction(object s, EventArgs ea)
		{
			...
			if(new PaySlipDialog(dg.SelectedRows[0].Cells[0].Value.ToString()).ShowDialog()==DialogResult.OK){
				ReloadData();
			}
		...
		public void DeleteRecordAction(object s, EventArgs ea)
		{
			...
			if(confirm==DialogResult.Yes){
				var sql = @"delete from thrdb.payslip where id=@p0";
				if(con.Execute(sql,dg.SelectedRows[0].Cells[0].Value)){
